...
| [ADR-0008](./0008-visual-orchestration-surface-for-the-fleet) | The **visual orchestration surface** — "ComfyUI/n8n of agent orchestration." **One command hub**: workstacean commands every agent — built-in (in-process) + distributed A2A (via the ADR-0007 gateway). Take the UX (live canvas, node palette, wiring, rendered workflows), keep **choreography** (nodes are reasoning agents, not data-flow boxes). One bus, no federation. Reframes ADR-0004 §8. |
