...
- **`workspace/mcp-servers.yaml`** — one list of `McpServerDef` entries (name, trust, transport stdio|sse, command/args/env or url, grants[], allowedTools/excludeTools, enabled, description). Control-plane-managed; the registrar owns the file (no hand-maintained comment block to protect, unlike `agents.yaml`, so a single file is fine — no `.d/` split needed).
