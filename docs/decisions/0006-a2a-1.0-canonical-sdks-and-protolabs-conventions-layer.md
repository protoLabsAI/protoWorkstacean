...
2. **Build the two conventions layers** — `@protolabs/a2a` (extract from the spike's extension/card/auth code) + `protolabs-a2a` (mirror, on `a2a-sdk`).
3. **Migrate the 4 Python repos** — delete hand-rolled handler → adopt `a2a-sdk` + `protolabs-a2a`. protoAgent first (roxy inherits the fork), then ORBIS, then pwnDeck. Each on a ready branch, not deployed.
