...
- **Custom DataParts** — discriminator stays in `metadata.mimeType`; payload in `content.value`:
  `{ content: { $case: "data", value: <payload> }, metadata: { mimeType: "https://proto-labs.ai/a2a/ext/<ext>" } }`
  for `cost-v1` (usage/costUsd/durationMs), `confidence-v1` (confidence/explanation), `worldstate-delta-v1`, `tool-call-v1` (per-tool progress frames). Declared in the card's `capabilities.extensions[]`.
