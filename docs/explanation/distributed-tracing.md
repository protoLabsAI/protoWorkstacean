...
## Flow example: Discord mention → protoPen → response
...
A2AExecutor calls the protoPen server:
  POST /a2a
  Headers:
...
protoPen processes, publishes response on agent.skill.response.trace-abc:
  bus message {
    id: "msg-003",
    correlationId: "trace-abc",
    parentId: "msg-002",    ← the A2A response is a child of the skill request
...
The A2A spec uses `contextId` in the response to carry the trace ID back. `A2AExecutor` sends `contextId: req.correlationId` in the request. External A2A agents (protoPen, ORBIS, protoContent) propagate this value in the response. workstacean reads it and confirms the response belongs to the right trace.
