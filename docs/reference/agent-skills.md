...
**41** tools are defined in the DeepAgent tool registry. Each agent's `tools:` list in `workspace/agents/<name>.yaml` selects which of these it exposes; a skill may further narrow the set via its own `tools:`.
