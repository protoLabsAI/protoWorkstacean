...
describe("GraphitiClient.getContextBlock", () => {
  it("returns empty string when no facts returned", async () => {
    ...
    const result = await client.getContextBlock("user_josh", "hello");
    expect(result).toBe("");
  });
  ...
  it("returns formatted block for active facts", async () => {
    ...
    const client = new GraphitiClient();
    const result = await client.getContextBlock("user_josh", "hello");
    expect(result).toContain("[User context — user_josh]");
  ...
  it("returns empty string when fetch throws", async () => {
    ...
    await expect(client.getContextBlock("user_josh", "hello")).rejects.toThrow("network error");
  });

  it("sends correct request body to /get-memory", async () => {
    spyFetch({ facts: [] });
    const client = new GraphitiClient();
    await client.getContextBlock("user_josh", "what is my name?");
...
describe("GraphitiClient.addEpisode", () => {
  it("posts to /messages with correct user and assistant turns", async () => {
    ...
    expect(body.group_id).toBe("user_josh");
  ...
  it("uses groupId as role when no userRole provided", async () => {
    ...
    const client = new GraphitiClient();
    await client.addEpisode({
      groupId: "user_discord_999",
      userMessage: "hi",
      agentMessage: "hello",
    });
    const body = lastFetchBody as { messages: Array<{ role: string }> };
    expect(body.messages[0]!.role).toBe("user_discord_999");
  });
...
describe("GraphitiClient.search", () => {
  it("returns facts array from /search", async () => {
    ...
    const client = new GraphitiClient();
    const facts = await client.search("user_josh", "typescript preferences", 5);
  ...
  it("returns empty array when no facts", async () => {
    mockFetch({ facts: [] });
    const client = new GraphitiClient();
    const facts = await client.search("user_josh", "anything");
  ...
  it("sends correct body to /search", async () => {
    spyFetch({ facts: [] });
    const client = new GraphitiClient();
    await client.search("user_josh", "my query", 7);
...
describe("GraphitiClient.clearUser", () => {
  it("calls DELETE /group/{groupId}", async () => {
    spyFetch(null, 200);
    const client = new GraphitiClient();
    await client.clearUser("user_josh");
    expect(lastFetchUrl).toContain("/group/user_josh");
  });

  it("throws on non-2xx response", async () => {
    mockFetch({ error: "not found" }, 404);
    const client = new GraphitiClient();
    await expect(client.clearUser("user_josh")).rejects.toThrow("404");
  });
});
