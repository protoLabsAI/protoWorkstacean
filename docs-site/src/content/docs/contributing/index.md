...
**Add a new plugin**: create `src/plugins/my-plugin.ts`, implement `Plugin`, wire it into `src/index.ts`. See [explanation/plugin-system.md](\1/) for the interface.

**Add a new executor type**: implement `IExecutor` in `src/executor/executors/`, export it, register it in the appropriate plugin's `install()`. See [reference/executor-types.md](\1/).
...
See [development.md](\1/) for install instructions, running tests, and the test structure.
