...
//     where consumers would see empty project state. The list is served by the
//     workstacean-projects nginx sidecar (GitHub `protoagent-plugin` topic +
//     an explicit base set, regenerated by sync-project-registry.sh). If the
//     sidecar is unreachable the registry keeps its last-good list and re-tries
//     every 5 min — consumers degrade gracefully (no project enrichment).
...
const pluginRegistry: PluginRegistryEntry[] = [
  ...
  },
  {
    ...
    // on same key in M min. Escalates to operator.message.request. Per-key
    // cooldown prevents DM flood.
